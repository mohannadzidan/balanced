...
export function resolveConstants(overrides: Partial<CostConstants> | undefined): CostConstants {
  return { ...DEFAULT_COST_CONSTANTS, ...overrides };
}

/** SPEC-v2.1 §4.1: tailroom = max(0, max over windows of (w.end − lengthMinutes)) */
export function computeTailroom(windows: readonly ResolvedWindow[], lengthMinutes: number): number {
  let maxExcess = 0;
