...
export function checkInvariants(timeline: Timeline): InvariantViolation[] {
  ...
  for (const inst of instances) {
    ...
    if (
      inst.scheduledMinutes > inst.durationMinutes &&
      !isSpanning &&
      !isExtended
    ) {
      violations.push({
  ...
  const anchorEnd = instances
    ...
    .reduce((max, i) => {
      ...
      if (i.state === "COMPLETED" && i.actualEnd !== null)
        return Math.max(max, i.actualEnd)
  ...
  for (const group of byOccurrenceId.values()) {
    ...
    for (const inst of group.slice(1)) {
      if (
        inst.activityId !== first.activityId ||
        inst.bucketKey !== first.bucketKey
      ) {
        violations.push({
  ...
  const rulesByActivityId = new Map<
    string,
    readonly TimelineActivity["rules"][number][]
  >()
  for (const inst of instances) {
  ...
  for (const inst of instances) {
    ...
    const rule = inst.rules.find(
      (r): r is import("./types").RepeatRule =>
        r.type === "repeat" && !r.sharedBudget
    )
  ...
  for (let i = 0; i < instances.length; i++) {
    ...
    for (let j = i + 1; j < instances.length; j++) {
      ...
      const minSep = Math.max(
        minSepByInstanceId.get(a.id)!,
        minSepByInstanceId.get(b.id)!
      )
