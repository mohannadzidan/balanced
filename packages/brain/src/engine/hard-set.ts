...
function elasticityFloorOf(activity: Activity): number {
  const rule = activity.rules.find((r): r is ElasticityRule => r.type === "elasticity");
 ...
 */
export function resolveFixedPlacement(rule: FixedRule, frame: DayFrame, dayIndex = 0): Placement {
...
export function placeFixedSet(
  ...
  const withFixed = activities
    ...
    .filter((x): x is { activity: Activity; placement: Placement } => x !== null);
  ...
  for (let i = 0; i < withFixed.length; i++) {
    ...
    if (baseOccupied.some((occ) => intervalsOverlap(withFixed[i].placement, occ))) {
      conflicted.add(withFixed[i].activity.id);
    }
  ...
  if (conflicted.size > 0) {
    ...
    diagnostics.push({
      ...
      suggestedFix: "Change one activity's fixed time, or remove the FixedRule from one of them.",
    });
  }
...
export function placeOccurrenceGroup(
  ...
  const minSeparationOf = (activity: Activity): number => {
    const rule = activity.rules.find((r): r is import("./types").RepeatRule => r.type === "repeat");
