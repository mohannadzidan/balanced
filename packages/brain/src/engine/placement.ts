...
export function enumerateFeasiblePlacementsForLength(
  ...
  const starts = enumerateCandidateStarts(length, context.freeIntervals, context.grid)
...
export function enumerateFeasiblePlacementsAcrossLengths(
  ...
  for (let length = fullLength; length >= floorMinutes; length -= context.grid) {
    for (const r of enumerateFeasiblePlacementsForLength(resolved, length, context)) {
...
export function placeActivityWithFloor(
  ...
  for (let length = fullLength; length >= floorMinutes; length -= context.grid) {
    const ranked = enumerateFeasiblePlacementsForLength(resolved, length, context);
