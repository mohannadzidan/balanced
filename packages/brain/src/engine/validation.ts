...
function checkRuleIncompatibility(activity: Activity, issues: ValidationIssue[]): void {
  ...
  for (let i = 0; i < rules.length; i++) {
    for (let j = i + 1; j < rules.length; j++) {
      ...
      // values differ.
      const duplicateType = a.type === b.type && a.type !== "window" && a.type !== "repeat";
      const forbiddenPair =
        (pairForbidden(a.type, b.type) && !(isDayOnlyWindow(a) || isDayOnlyWindow(b))) ||
...
function checkGridAlignment(
  ...
  for (const rule of activity.rules) {
    ...
    if (rule.type === "elasticity") {
      if (!isOnGrid(rule.minTotalMinutes, grid) || !isOnGrid(rule.minBlockMinutes, grid)) {
        issues.push(
          issue(
            "error",
            "DURATION_NOT_ON_GRID",
...
function checkRepeatDuplicate(activity: Activity, issues: ValidationIssue[]): void {
  const repeats = activity.rules.filter((r): r is RepeatRule => r.type === "repeat");
 ...
 */
function checkFixedWithMultiCount(activity: Activity, issues: ValidationIssue[]): void {
...
export function validateActivity(activity: Activity, constants: CostConstants): ValidationIssue[] {
  ...
  for (const rule of activity.rules as readonly Rule[]) {
    if (rule.type === "elasticity") checkElasticityInvalid(activity, rule, issues);
...
function checkSequenceMultiple(activities: readonly Activity[], issues: ValidationIssue[]): void {
...
function checkSequenceCycle(activities: readonly Activity[], issues: ValidationIssue[]): void {
...
function checkGuestOutranksHost(activities: readonly Activity[], issues: ValidationIssue[]): void {
  const byId = new Map(activities.map((a) => [a.id, a]));
...
export function validateCatalog(activities: readonly Activity[]): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
 ...
 */
export function validateSeparation(activity: Activity, dayFrame: DayFrame): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
...
export function validateFrame(frame: DayFrame): ValidationIssue[] {
  ...
  if (frame.dayCount > 366) {
    issues.push(
      issue("error", "FRAME_TOO_LONG", null, `Frame spans ${frame.dayCount} days; the cap is 366.`),
    );
  }
  ...
  if (
    ...
    (!Number.isFinite(frame.backdateHorizonMinutes) || frame.backdateHorizonMinutes < 0)
  ) {
    issues.push(
      issue(
        "error",
