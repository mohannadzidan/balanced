...
import { validateActivity, validateCatalog, validateSeparation } from "../src/engine/validation";
...
describe("validateActivity", () => {
  ...
  it.each([
    [
      "fixed + strictWindow",
      (a: ReturnType<typeof activity>) => a.fixed("09:00", "10:00").strict("09:00", "10:00"),
    ],
    [
      "fixed + flexibleWindow",
      (a: ReturnType<typeof activity>) => a.fixed("09:00", "10:00").flexible("09:00", "10:00"),
    ],
    ...
    [
      "fixed + shrink",
      (a: ReturnType<typeof activity>) => a.fixed("09:00", "10:00").shrink({ floor: 30 }),
    ],
  ...
  it("flags DURATION_NOT_ON_GRID for an off-grid window boundary", () => {
    const bad = activity("Bad").rank(1).minutes(60).strict("09:03", "10:00").build();
    expect(codesOf(validateActivity(bad, C))).toContain("DURATION_NOT_ON_GRID");
  });

  it("flags DURATION_NOT_ON_GRID for an off-grid shrink floor", () => {
    const bad = activity("Bad").rank(1).minutes(60).shrink({ floor: 47 }).build();
    expect(codesOf(validateActivity(bad, C))).toContain("DURATION_NOT_ON_GRID");
  });
  ...
  it("flags ELASTICITY_INVALID when the floor exceeds the duration", () => {
    const bad = activity("Bad").rank(1).minutes(60).shrink({ floor: 90 }).build();
  ...
  it("no longer flags NOT_YET_SUPPORTED for minSeparationMinutes > 0 (lifted in SPEC-v2.1 §13.1 step 4)", () => {
    ...
    expect(codesOf(validateActivity(withRepeat, C))).not.toContain("NOT_YET_SUPPORTED");
  });
  ...
  it("flags SEPARATION_UNSATISFIABLE when minSeparationMinutes makes even the day bucket infeasible", () => {
    ...
    expect(codesOf(validateActivity(withRepeat, C))).not.toContain("NOT_YET_SUPPORTED");
    expect(codesOf(validateSeparation(withRepeat, frame))).toContain("SEPARATION_UNSATISFIABLE");
  });
  ...
  it("flags REPEAT_DUPLICATE for two RepeatRules with the same sharedBudget value", () => {
    ...
    expect(codesOf(validateActivity(withDuplicateRepeats, C))).toContain("REPEAT_DUPLICATE");
  });
  ...
  it("flags FIXED_WITH_MULTI_COUNT when a FixedRule activity has a recurrence RepeatRule count > 1 (SPEC-v2.1 §7.1)", () => {
    const bad = activity("Standup")
      ...
      .build();
    expect(codesOf(validateActivity(bad, C))).toContain("FIXED_WITH_MULTI_COUNT");
  });
  ...
  it("does not flag FIXED_WITH_MULTI_COUNT for a FixedRule activity with a recurrence RepeatRule count of 1", () => {
    const ok = activity("Standup")
      ...
      .build();
    expect(codesOf(validateActivity(ok, C))).not.toContain("FIXED_WITH_MULTI_COUNT");
  });
  ...
  it("flags WINDOW_INVERTED when a strict window ends before it starts", () => {
    const bad = activity("Bad").rank(1).minutes(30).strict("10:00", "09:00").build();
  ...
  it("flags WINDOW_TOO_SHORT when a strict window can't fit the duration and there's no shrink", () => {
    const bad = activity("Bad").rank(1).minutes(90).strict("09:00", "10:00").build();
  ...
  it.each([-1, 2])(
    ...
    (n) => {
      ...
      expect(codesOf(validateActivity(bad, C))).toContain("REQUIRED_COUNT_INVALID");
    },
  );

  it.each([0, 1])("does not flag REQUIRED_COUNT_INVALID for requiredCount %i", (n) => {
    const ok = activity("Ok").rank(1).minutes(30).required(n).build();
    expect(codesOf(validateActivity(ok, C))).not.toContain("REQUIRED_COUNT_INVALID");
  });
  ...
  it("flags DOMINANCE_VIOLATION for a deliberately broken activity", () => {
    ...
    expect(codesOf(validateActivity(broken, C))).toContain("DOMINANCE_VIOLATION");
  });
});
...
describe("validateCatalog", () => {
  ...
  it("does not flag GUEST_OUTRANKS_HOST when the host outranks its guest", () => {
    ...
    expect(codesOf(validateCatalog(catalog))).not.toContain("GUEST_OUTRANKS_HOST");
  });
});
