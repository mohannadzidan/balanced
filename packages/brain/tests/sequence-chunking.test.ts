...
describe("solve — sequence dependents bind to a chunked host's first/last chunk (SPEC.md 11, edge case 12)", () => {
  it("binds the pre-dependent to the first chunk's own start and the post-dependent to the last chunk's own end", () => {
    const catalog = [
      ...
      activity("Evening Meeting").rank(2).minutes(120).fixed("15:30", "17:30").build(),
      activity("Deep Work")
        .rank(3)
        .minutes(120)
        ...
        .build(),
      activity("Commute").rank(4).minutes(15).sequence("pre", "deep-work").build(),
      activity("Cooldown").rank(5).minutes(15).sequence("post", "deep-work").build(),
    ];
    ...
    const cooldown = result.timeline.instances.find((i) => i.name === "Cooldown")!;
