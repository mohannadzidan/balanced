...
describe("solve — SKIP (SPEC.md 9.7)", () => {
  it("marks a PLANNED instance user-skipped, freeing its time for a lower-priority activity", () => {
    ...
    const skippedWork = result.timeline.instances.find((i) => i.name === "Work")!;
  ...
  it("keeps a user skip pinned across a later TICK, unlike an ordinary auto-skip", () => {
    ...
    const stillSkipped = ticked.timeline.instances.find((i) => i.name === "Work")!;
...
describe("solve — RESTORE (SPEC.md 9.7)", () => {
  it("lifts a user skip and re-solves the activity back into the day", () => {
    ...
    const skippedWork = skipped.timeline.instances.find((i) => i.name === "Work")!;
    ...
    const restoredWork = restored.timeline.instances.find((i) => i.name === "Work")!;
  ...
  it("can displace and reject a different, previously-fine sequence dependent (SPEC.md 9.7's hedge)", () => {
    ...
    const catalog = [
      activity("Work").rank(1).minutes(60).build(),
      activity("Report").rank(2).minutes(30).sequence("post", "work", { maxGap: 0 }).build(),
      activity("Restore").rank(3).minutes(60).mandatory().strict("00:00", "01:00").build(),
    ...
    const work = generated.timeline.instances.find((i) => i.name === "Work")!;
    const report = generated.timeline.instances.find((i) => i.name === "Report")!;
    const restore = generated.timeline.instances.find((i) => i.name === "Restore")!;
