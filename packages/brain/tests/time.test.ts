...
describe("resolveDayFrame", () => {
  it("is 1440 minutes on an ordinary day", () => {
    expect(resolveDayFrame("2024-06-15", "America/New_York").lengthMinutes).toBe(1440);
  });
  ...
  it("is 1380 minutes on the spring-forward day", () => {
    ...
    expect(resolveDayFrame("2024-03-10", "America/New_York").lengthMinutes).toBe(1380);
  });

  it("is 1500 minutes on the fall-back day", () => {
    // America/New_York: 2024-11-03, clocks fall back 02:00 -> 01:00
    expect(resolveDayFrame("2024-11-03", "America/New_York").lengthMinutes).toBe(1500);
  });
...
describe("resolveWallClockToInstant — DST edge cases", () => {
  it("resolves an unambiguous time normally", () => {
    const result = resolveWallClockToInstant(2024, 6, 15, 9, 0, "America/New_York");
  ...
  it("resolves a non-existent spring-forward time to the transition instant", () => {
    ...
    const result = resolveWallClockToInstant(2024, 3, 10, 2, 30, "America/New_York");
  ...
  it("resolves an ambiguous fall-back time to its first occurrence", () => {
    ...
    const result = resolveWallClockToInstant(2024, 11, 3, 1, 30, "America/New_York");
