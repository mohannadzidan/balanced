...
describe("computeFreeIntervals", () => {
  ...
  it("clips occupied intervals to the requested range", () => {
    ...
    expect(computeFreeIntervals(occupied, 0, 1440)).toEqual([{ start: 50, end: 1400 }]);
  });

  it("ignores occupied intervals entirely outside the range", () => {
    const occupied = [{ start: -100, end: -10 }];
    expect(computeFreeIntervals(occupied, 0, 100)).toEqual([{ start: 0, end: 100 }]);
  });
...
describe("intervalsOverlap", () => {
  it.each([
    ["disjoint, a before b", { start: 0, end: 10 }, { start: 10, end: 20 }, false],
    ["disjoint, b before a", { start: 20, end: 30 }, { start: 0, end: 10 }, false],
...
describe("intervalContains", () => {
  it("is true when inner lies entirely within outer", () => {
    expect(intervalContains({ start: 0, end: 100 }, { start: 10, end: 20 })).toBe(true);
  });

  it("is true for exact boundary equality", () => {
    expect(intervalContains({ start: 0, end: 100 }, { start: 0, end: 100 })).toBe(true);
  });

  it("is false when inner spills past outer", () => {
    expect(intervalContains({ start: 0, end: 100 }, { start: 90, end: 110 })).toBe(false);
  });
});
