...
function toComparable(
  ...
  return instances
    .map((i) => ({
      ...
      date: i.date,
      relativeStart: i.plannedStart === null ? null : i.plannedStart - dayStartOffsetOf(i.date),
      relativeEnd: i.plannedEnd === null ? null : i.plannedEnd - dayStartOffsetOf(i.date),
      state: i.state,
      scheduledMinutes: i.scheduledMinutes,
    }))
    .sort(
      (a, b) =>
        (a.activityId ?? "").localeCompare(b.activityId ?? "") || a.date.localeCompare(b.date),
    );
}
    ...
    fc.assert(
      fc.property(
        ...
        (startDate, catalog, dayCount) => {
          ...
          const big = toComparable(bigResult.timeline.instances, dayStartOffsetOf);
