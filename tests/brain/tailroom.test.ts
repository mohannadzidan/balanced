...
describe("computeTailroom (SPEC-v2.1 §4.1)", () => {
	it("returns 0 when no window extends past lengthMinutes", () => {
		const windows: ResolvedWindow[] = [
			{ start: 0, end: 60, maxDriftMinutes: 0, dayIndex: 0, daySpanStart: 0, daySpanEnd: 1440 },
			{ start: 100, end: 200, maxDriftMinutes: 0, dayIndex: 0, daySpanStart: 0, daySpanEnd: 1440 },
		]
	...
	it("returns w.end − lengthMinutes when a window extends past lengthMinutes", () => {
		const windows: ResolvedWindow[] = [
			{ start: 1380, end: 1860, maxDriftMinutes: 0, dayIndex: 0, daySpanStart: 0, daySpanEnd: 2880 },
		]
		// 1860 − 1440 = 420.
	...
	it("returns the maximum excess across multiple windows", () => {
		const windows: ResolvedWindow[] = [
			{ start: 1380, end: 1500, maxDriftMinutes: 0, dayIndex: 0, daySpanStart: 0, daySpanEnd: 2880 }, // excess 60
			{ start: 1000, end: 1700, maxDriftMinutes: 0, dayIndex: 0, daySpanStart: 0, daySpanEnd: 2880 }, // excess 260
			{ start: 1380, end: 1860, maxDriftMinutes: 0, dayIndex: 0, daySpanStart: 0, daySpanEnd: 2880 }, // excess 420 (max)
		]
		expect(computeTailroom(windows, 1440)).toBe(420)
	})
	...
	it("ignores windows that end exactly at lengthMinutes", () => {
		const windows: ResolvedWindow[] = [
			{ start: 0, end: 1440, maxDriftMinutes: 0, dayIndex: 0, daySpanStart: 0, daySpanEnd: 1440 },
		]
		expect(computeTailroom(windows, 1440)).toBe(0)
	})
})