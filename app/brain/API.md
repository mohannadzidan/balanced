...
surface exported from `@/app/brain/brain` — the only module callers should
import from. `app/brain/engine/*` is implementation detail (placement
search, backtracking, cost internals) and is not a supported import path
outside `app/brain/engine` itself. For the full behavioral spec (why the
solver does what it does), see `app/brain/SPEC.md`; this document only
...
All of the following are exported as types from `@/app/brain/brain` (they
mirror `app/brain/engine/types.ts`, which is the source of truth if this
drifts):
...
Everything under `app/brain/engine/` besides the files re-exported above
...
this page, that's a sign to extend `app/brain/brain.ts`'s barrel rather
