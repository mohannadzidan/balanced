...
import {
  ...
  placeSequenceChain,
  sequenceRuleOf,
} from "@/app/brain/engine/sequence"
import { resolveDayFrame } from "@/app/brain/engine/time"
import type { Activity, SequenceRule } from "@/app/brain/engine/types"
...
describe("placeSequenceChain", () => {
  ...
  it("places a dependent adjacent to its placed host", () => {
    const commute = activity("Commute")
      ...
      .sequence("pre", "work")
      .build()
    const hostResolutions = new Map([
      ["work", { start: 540, end: 600, nestedIn: null }],
    ] as const)
  ...
  it("resolves a chain (A pre B, B pre C) across rounds", () => {
    ...
    const hostResolutions = new Map([
      ["c", { start: 600, end: 660, nestedIn: null }],
    ] as const)
  ...
  it("skips with NO_FREE_SPACE when the host is placed but no adjacent room exists", () => {
    ...
    const outcome = placeSequenceChain(
      [commute],
      hostResolutions,
      baseOccupied,
      ctx
