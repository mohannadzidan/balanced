...
export function OneOffEventSheet() {
  ...
  function handleSubmit(formData: FormData) {
    startTransition(async () => {
      const result = await createOneOffActivityAction({ ok: false, error: "" }, formData)
      if (result.ok) {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger
        render={<Button variant="outline" className="flex-1 flex items-center justify-center gap-2 shadow-whisper" />}
      >
      ...
      >
        ...
        <SheetHeader>
          ...
          <SheetDescription>
            Exists only on today&apos;s timeline — your global activity templates are untouched.
          </SheetDescription>
        </SheetHeader>
        <form ref={formRef} action={handleSubmit} className="flex flex-col gap-4 px-4">
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="one-off-title">Name</Label>
            <Input id="one-off-title" name="title" placeholder="Pick up dry cleaning" required />
          </div>
          <div className="flex gap-2">
            <div className="flex flex-1 flex-col gap-1.5">
              <Label htmlFor="one-off-start">Start</Label>
              <Input id="one-off-start" name="startMin" type="time" defaultValue="09:00" required />
            </div>
            <div className="flex flex-1 flex-col gap-1.5">
              <Label htmlFor="one-off-end">End</Label>
              <Input id="one-off-end" name="endMin" type="time" defaultValue="09:30" required />
            </div>
          </div>
